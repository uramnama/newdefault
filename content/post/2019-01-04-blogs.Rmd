...
```{r animate, echo=FALSE,warning=FALSE}
library(gganimate)
library(gapminder)
a = head(gapminder,35)
options(scipen = 999)
ggplot(a) + geom_point(aes(gdpPercap,lifeExp,size=pop,colour=country)) +
  transition_time(year) + labs(title = 'year: {frame_time}') + theme_bw() + facet_wrap(~country)
